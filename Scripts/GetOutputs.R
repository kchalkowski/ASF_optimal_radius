...
GetOutputs<-function(pop,Incidence,Tculled,ICtrue,out,detectday,out.opts,...){
...
if("alldetections"%in%out.opts){
  ...
  detections=matrix(nrow=0,ncol=5)
  for(i in 1:length(POSlive)){
    detections.i=matrix(nrow=length(c(POSlive_locs[[i]])),ncol=5)
    detections.i[,1]=i
    detections.i[,2]=POSlive[[i]]
    detections.i[,3]=c(POSlive_locs[[i]])
    detections.i[,4]=POSdead[[i]]
    detections.i[,5]=c(POSdead_locs[[i]])
  }
